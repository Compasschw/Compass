...
async def deactivate_account(
    ...
    """Member-initiated account deactivation — reversible, data-preserving.

    Distinct from ``DELETE /api/v1/auth/users/me`` (hard delete, irreversible).
    This endpoint flips ``is_active=False`` and revokes all refresh tokens so
    the member is logged out everywhere, but every PHI/PII row is left fully
    intact. See ``account_deletion.deactivate_member_account`` for the
    reactivation-TBD note (owner: JT) — there is currently no self-service or
    admin reactivation path; this endpoint is one-directional until that ships.
    """
    from app.services.account_deletion import deactivate_member_account

    await deactivate_member_account(db, current_user)
    await db.commit()
    return None
